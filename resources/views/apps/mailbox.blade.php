                        ...
                        <div class="p-4 relative">
                            <div class="flex flex-wrap">
                                <div class="flex-shrink-0 ltr:mr-2 rtl:ml-2">
                                    <img x-show="selectedMail.path" :src="`/assets/images/${selectedMail.path}`"
                                ...
                                <div>
                                    <div class="flex items-center justify-center space-x-3 rtl:space-x-reverse">
                                        <button x-tooltip="Star" data-placement="top" role="tooltip" type="button"
                                        ...
                                        <button x-tooltip="Reply" data-placement="top" role="tooltip" type="button"
                                            class="hover:text-info" @click="openMail('reply', selectedMail)">
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data("mailbox", () => ({
                ...
                checkAllCheckbox() {
                    return this.filteredMailList.length && this.ids.length === this.filteredMailList
                        .length;
                },
                ...
                showTime(item) {
                    ...
                    if (displayDt.toDateString() === cDt.toDateString()) {
                        ...
                        return String(displayDt.getMonth() + 1).padStart(2, '0') + '/' +
                ...
                checkAll(isChecked) {
                    if (isChecked) {
                        this.ids = this.filteredMailList.map((d) => d.id);
                    } else {
                        this.clearSelection();
                    }
                },
                ...
                searchMails(isResetPage = true) {
                    ...
                    } else if (['personal', 'work', 'social', 'private'].includes(this.selectedTab)) {
                    ...
                    if (this.filteredMailList.length) {
                        this.pager.totalPages = this.pager.pageSize < 1 ?
                            1 : Math.ceil(this.filteredMailList.length / this.pager.pageSize);
                        ...
                        this.pagedMails = this.filteredMailList.slice(
                ...
                setImportant(mailId) {
                    const item = this.filteredMailList.find((d) => d.id === mailId);
                    if (item) {
                        item.isImportant = !item.isImportant;
                        setTimeout(() => this.searchMails(false));
                    }
                },
                ...
                setSpam() {
                    if (this.ids.length) {
                        const items = this.filteredMailList.filter(d => this.ids.includes(d.id));
                        for (const item of items) {
                            item.type = item.type === 'spam' ? 'inbox' : 'spam';
                        }
                        this.showMessage(
                            `${this.ids.length} Mail has been ${this.selectedTab === 'spam' ? 'removed from' : 'added to'} Spam.`
                            );
                        this.searchMails(false);
                    }
                },
                ...
                setArchive() {
                    if (this.ids.length) {
                        ...
                        this.showMessage(
                            `${this.ids.length} Mail has been ${this.selectedTab === 'archive' ? 'removed from' : 'added to'} Archive.`
                            );
                        this.searchMails(false);
                    }
                },
                ...
                setGroup(group) {
                    if (this.ids.length) {
                        ...
                        this.showMessage(
                            `${this.ids.length} Mail has been grouped as ${group.toUpperCase()}`);
                        this.clearSelection();
                        setTimeout(() => this.searchMails(false));
                    }
                },
                        ...
                        for (const item of items) {
                            switch (type) {
                                case 'trash':
                                    ...
                                    this.showMessage(`${this.ids.length} Mail has been deleted.`);
                                    break;
                                case 'read':
                                    item.isUnread = false;
                                    this.showMessage(`${this.ids.length} Mail marked as Read.`);
                                    break;
                                case 'unread':
                                    item.isUnread = true;
                                    this.showMessage(`${this.ids.length} Mail marked as UnRead.`);
                                    break;
                                case 'important':
                                    item.isImportant = true;
                                    this.showMessage(`${this.ids.length} Mail marked as Important.`);
                                    break;
                                case 'unimportant':
                                    item.isImportant = false;
                                    this.showMessage(`${this.ids.length} Mail marked as UnImportant.`);
                                    break;
                                case 'star':
                                    item.isStar = true;
                                    this.showMessage(`${this.ids.length} Mail marked as Star.`);
                                    break;
                                case 'restore':
                                    item.type = 'inbox';
                                    this.showMessage(`${this.ids.length} Mail Restored.`);
                                    break;
                                case 'delete':
                                    this.mailList = this.mailList.filter(d => !this.ids.includes(d.id));
                                    this.showMessage(`${this.ids.length} Mail Permanently Deleted.`);
                                    break;
                            }
                        }
                        this.clearSelection();
                ...
                saveMail(type, id = null) {
                    ...
                    let maxId = this.mailList.reduce((max, d) => d.id > max ? d.id : max, 0);
                    let cDt = new Date();
                    ...
                    let obj = {
                        ...
                        date: `${cDt.getFullYear()}-${String(cDt.getMonth() + 1).padStart(2, '0')}-${String(cDt.getDate()).padStart(2, '0')}`,
                        time: cDt.toLocaleTimeString(),
                        title: this.params.title,
                        displayDescription: this.quillEditor.getText(),
                        type: (type === 'save' || type === 'save_reply' || type ===
                            'save_forward') ? 'draft' : 'sent_mail',
                        isImportant: false,
                        group: '',
                        isUnread: false,
                    ...
                    if (this.params.file && this.params.file.length) {
                        for (let file of this.params.file) {
                            obj.attachments.push({
                                name: file.name,
                                size: this.getFileType(file.size),
                                type: this.getFileSize(file.type)
                            });
                        }
                    }

                    this.mailList.unshift(obj);
                    this.searchMails();

                    this.showMessage(
                        `Mail has been ${type.startsWith('save') ? 'saved to Draft' : 'sent successfully'}.`
                        );
                    this.selectedMail = null;
                    this.isEdit = false;
                },
                ...
                getFileSize(file_type) {
                    ...
                    if (file_type.includes('application/x-zip')) return 'zip';
                    return 'file';
                },

                getFileType(total_bytes) {
                    return total_bytes < 1000000 ? Math.floor(total_bytes / 1000) + 'KB' : Math.floor(
                        total_bytes / 1000000) + 'MB';
                },
                ...
                openMail(type, item = null) {
                    ...
                    if (type === 'add') {
                        ...
                        if (this.quillEditor) this.quillEditor.setText('');
                    } else if (['draft', 'reply', 'forward'].includes(type)) {
                        ...
                        if (type === 'reply') {
                            this.params.title = `Re: ${data.title}`;
                            this.params.displayDescription = `Re: ${data.title}`;
                        } else if (type === 'forward') {
                            this.params.title = `Fwd: ${data.title}`;
                            this.params.displayDescription = `Fwd: ${data.title}`;
                        }
                    }
